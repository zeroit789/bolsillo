...
/* =============================================================================
 * Onboarding.vue — Pantalla de bienvenida inicial / First-run welcome screen
 * -----------------------------------------------------------------------------
 ...
 *   2. Diccionario de textos de UI (i18n) / UI text dictionary (i18n)
 *   3. Estado reactivo del formulario / Reactive form state
 *   4. Manejador de envío (validación + fin) / Submit handler (validation + finish)
 * ===========================================================================*/
...
// ── 3. Estado reactivo del formulario / Reactive form state ───────────────────
// ES: Nombre del usuario (texto libre, opcional). / EN: User's name (free text, optional).
const nombre = ref("");
...
// ES: Mensaje de error / EN: Current validation error message (empty = no error).
//     de validación actual (vacío = sin error).
const error = ref("");
// ES: Bandera de / EN: Loading flag: true while finishing, to disable the button.
//     carga: true mientras se termina, para deshabilitar el botón.
const cargando = ref(false);

// ES: El PIN debe tener entre 4 y 6 dígitos. / EN: PIN must be 4 to 6 digits.
...
async function empezar() {
  error.value = "";
  // ES: Validar el bloqueo solo si se eligió. / EN: Validate the lock only if one was chosen.
  if (pideCredencial.value) {
    // ES: PIN: exigir el formato de 4-6 dígitos. / EN: PIN: enforce the 4-6 digit format.
    if (tipoBloqueo.value === "pin" && !REGEX_PIN.test(credencial.value)) {
      error.value = t("errorPin");
      return;
    }
    // ES: Contraseña: exigir longitud mínima. / EN: Password: enforce a minimum length.
    if (tipoBloqueo.value === "password" && credencial.value.length < 4) {
      error.value = t("errorPassword");
      return;
    }
    // ES: Ambos campos deben coincidir. / EN: Both fields must match.
  ...
  cargando.value = true;
  // ES: Si se eligió fecha / EN: If a manual date was chosen, start in that month.
  //     manual, arrancamos en el mes indicado.
  if (modoFecha.value === "manual" && mesElegido.value) {
    finanzas.seleccionarMes(mesElegido.value);
  }
  // ES: Guardar nombre / EN: Persist name and lock (null = open directly, no lock).
...
<template>
  ...
  <div class="min-h-screen flex items-center justify-center bg-base p-6">
    <div class="w-full max-w-md rounded-2xl bg-surface border border-border p-7">
      ...
      <div class="space-y-4">
        <!-- ES: Campo de nombre (texto libre). / EN: Name field (free text). -->
        <div>
        ...
        <div>
          ...
          <template v-if="pideCredencial">
            <!-- ES: Introducción de la credencial. / EN: Credential entry. -->
            <input
              v-model="credencial"
