...
/* =============================================================================
 * PresupuestosView.vue — Topes mensuales por categoría / Monthly budget caps per category
 * -----------------------------------------------------------------------------
 ...
 *   2. Traducciones (textos de UI) / Translations (UI texts)
 *   3. Estado del formulario / Form state
...
function colorBarra(categoria: string, limite: number): string {
  ...
  if (pct > 100) return "bg-danger"; // ES: rojo — pasado / EN: red — over budget
  if (pct >= 80) return "bg-warn"; // ES: amarillo — al borde / EN: yellow — near the cap
  return "bg-ok"; // ES: verde — tranquilo / EN: green — relaxed
}
...
function guardarTope(): void {
  ...
  // ES: Debe haber una categoría elegida. / EN: A category must be chosen.
  if (!cat) {
...
<template>
  ...
  <div class="min-h-full bg-base p-6 text-ink">
    ...
    <!-- ES: 2. Formulario para añadir / editar un tope / EN: 2. Form to add / edit a cap -->
    <section class="rounded-2xl bg-surface border border-border p-5 mb-6">
      <h2 class="font-display font-bold mb-4">{{ t("nuevoTope") }}</h2>
      <!-- ES: @submit.prevent para no recargar la SPA de escritorio / EN: @submit.prevent so the desktop SPA doesn't reload -->
      <form class="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end" @submit.prevent="guardarTope">
        <!-- ES: Selector de categoría / EN: Category selector -->
        <div>
        ...
        <!-- ES: Importe del tope (€) / EN: Cap amount (€) -->
        <div>
        ...
        <!-- ES: Botón guardar / EN: Save button -->
        <button
          type="submit"
    ...
    <!-- ES: 3. Estado vacío: no hay presupuestos definidos / EN: 3. Empty state: no budgets defined -->
    <div
    ...
    <!-- ES: 4. Listado de presupuestos definidos: una tarjeta por categoría / EN: 4. List of defined budgets: one card per category -->
    <div v-else class="grid gap-4 sm:grid-cols-2">
      ...
      >
        <!-- ES: Cabecera de la tarjeta: punto de color + categoría + botón eliminar / EN: Card header: colour dot + category + delete button -->
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-center gap-2 min-w-0">
            <!-- ES: Punto del color de la categoría / EN: Category colour dot -->
            <span
          ...
          <!-- ES: Botón eliminar (con confirm) / EN: Delete button (with confirm) -->
          <button
        ...
        <!-- ES: Barra de progreso gastado / límite / EN: Progress bar spent / cap -->
        <div class="mt-4">
          <div class="h-2 w-full overflow-hidden rounded-full bg-surface-2">
            <!-- ES: Relleno: ancho recortado a 0..100, color según el % gastado / EN: Fill: width clamped to 0..100, colour by % spent -->
            <div
          ...
          <!-- ES: Aviso cuando se ha pasado del tope / EN: Notice when the cap has been exceeded -->
